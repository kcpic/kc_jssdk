...
class SignatureForm {
  ...
  async connect() {	
	...
	try {
	  ...
	  } else if (this.mInformation.idProduct != "STU-300") {
  ...
  async getCaptureData() {
    ...
    var device = {
        ...
        'device_pixels_per_m_x': this.mCapability.tabletMaxX * 10,
		'device_pixels_per_m_y': this.mCapability.tabletMaxY * 10,
        'device_origin_X': 0,
        'device_origin_Y': 1,
		'has_tilt': false,
		'has_twist': false
    ...
    await this.sigObj.generateSignature(who, why, where, Module.KeyType.SHA512, hash, strokeVector, device, digitzerInfo, nicInfo, timeResolution, new Date());
    hash.delete();
    strokeVector.delete();
    currentStroke.delete();
  ...
  onPenData(penData, time) { // Process incoming pen data
    ...
    {
      var isDown = !(penData.pressure <= this.mInkThreshold.offPressureMark);
